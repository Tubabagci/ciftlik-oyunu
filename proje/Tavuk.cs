...
namespace proje
{
    class Tavuk : AbstractHayvan, IDepo, ISatis, IKazanc
    {
        private SoundPlayer tavukOlumSesi = new SoundPlayer();
        public Tavuk()//Kurucu tanımlanır.
        {
            tavukOlumSesi.SoundLocation = Application.StartupPath + "\\tavuk.wav";//Ölüm sesi eklenir.
        ...
        {
            {
                if (enerji == 2)
                {
                    tavukOlumSesi.Play();//ÖLüm sesi çağırılır
                    enerji = 0;
        ...
        public void UrunMiktar(string mevcutUrunMiktari, int uzunluk, int sure, int enerji)
        {
            ...
            if (enerji > 0)
            {
                if (sure > 0 && sure % 3 == 0)// Süre 3'ün katı olduğunda yeni ürün üretir
                {
        ...
        public void Kazanc(string mevcutUrunMiktari, int uzunluk, string kasa)
        {
            ...
            if (urunMiktari > 0)
            {
                ...
                kazanılan +=urunMiktari*1;//fiyata ve ürün miktarına göre yei kazanılan para hesaplanır.
